...
module.exports = class Classroom {
    ...
    constructor({config, cortex, managers, validators, mongomodels} = {}) {
        ...
        this.roles = {
            ...
            // ,
            // archiveClassroom: ['SCHOOL_ADMIN'],
            // getClassroomRoster: ['SCHOOL_ADMIN']
        };
    }
     ...
     */
    async createClassroom({__authenticate, __authorize, name, grade, capacity, classTeacher, resources}) {
        try {
            ...
            const validation = await this.validators.classroom.createClassroom({
                name, grade, capacity, classTeacher, resources
            });
            ...
            if (!school) {
                return errorHandlers.notFoundError('School not found for the authenticated user');
            }
            ...
            return successHandlers.created(classroom, 'Classroom created successfully');
        } catch (error) {
            return errorHandlers.serverError(`Failed to create classroom: ${error.message}`);
        }
    }

    async getSchoolByAdminId(__authenticate) {
        let school = null;
        const user = await this.userManager.getUserById(__authenticate.userId);
        if (!user) {
            console.error('User not found for ID:', __authenticate.userId);
            return null;
        }
        school = await this.schoolManager.getSchoolById(user.schoolId);
        if (!school) {
    ...
    async getAllClassrooms({__authenticate, __authorize, __query}) {
        try {
            ...
            return errorHandlers.serverError(`Failed to retrieve classrooms: ${error.message}`);
        }
    }
     ...
     * @throws {404} If school not found
     */
    async getSchoolClassrooms ({__authenticate, __authorize, __params,__query}) {
        try {
            //get school
            const school = await this.schoolManager.getSchoolById(__params.id);
    ...
    async getById({__authenticate, __authorize, __params}) {
        try {
            ...
            const classroom = await this.getClassroomByIdWithRoleCheck(classroomId, __authenticate, __authorize);

            if (!classroom) {
                return errorHandlers.notFoundError('Classroom not found');
            }

            return successHandlers.success(classroom, 'Classroom retrieved successfully');
        } catch (error) {
            return errorHandlers.serverError(`Failed to retrieve classroom: ${error.message}`);
        }
    }
     ...
     * @throws {404} If classroom not found
     */
    async updateClassroom({__authenticate, __authorize, __params, name, grade, capacity, classTeacher, resources}) {
        try {
            const classroomId = __params.id;
            const userId = __authenticate.userId;
            ...
            return successHandlers.updated(updatedClassroom, 'Classroom updated successfully');
        } catch (error) {
            return errorHandlers.serverError(`Failed to update classroom: ${error.message}`);
        }
    }
    ...
    //  * Only SCHOOL_ADMIN can archive (their own school)
    //  * @param {string} schoolId - School ID
    //  * @param {string} classroomId - Classroom ID
    //  * @param {string} updatedByUserId - ID of admin archiving
    ...
    //             return errorHandlers.conflictError(`Cannot archive classroom with ${activeStudentsCount} active student(s)`);
    //         }

    //         const userId = __authenticate.userId;
    //         const classroom = await this.Classroom.findOneAndUpdate(
    ...
    //         return successHandlers.updated(classroom, 'Classroom archived successfully');
    //     } catch (error) {
    //         return errorHandlers.serverError(`Failed to archive classroom: ${error.message}`);
    //     }
    // }
    ...
    // /**
    //  * Get classroom roster - all active students in classroom
    //  * Only SCHOOL_ADMIN can view (their own school)
    //  * @param {string} schoolId - School ID
    //  * @param {string} classroomId - Classroom ID
    //  * @returns {Array} Array of students in classroom
    //  */
    ...
    //         const studentClassrooms = await this.StudentClassroom.find({
    //             classroomId,
    //             schoolId,
    //             isActive: true
    //         })
    ...
    //         return successHandlers.success(roster, 'Classroom roster retrieved successfully');
    //     } catch (error) {
    //         return errorHandlers.serverError(`Failed to retrieve classroom roster: ${error.message}`);
    //     }
    // }
   ...
   * @returns {Object|null} Classroom object or null if not found
   */
  async getClassroomByIdWithRoleCheck(classroomId, __authenticate, __authorize) {
    let classroom = null;
